...
from ._helpers import default_base_url, get_auth_header, is_processing_interrupted, sleep_with_interrupt
...
async def download_url_to_bytesio(
    url: str,
    dest: BytesIO | str | Path | None,
    *,
    timeout: float | None = None,
    max_retries: int = 5,
    retry_delay: float = 1.0,
    retry_backoff: float = 2.0,
    cls: type[COMFY_IO.ComfyNode] | None = None,
) -> None:
    ...
    if not parsed_url.scheme and not parsed_url.netloc:
        if cls is None:
            raise ValueError("For relative API paths, the `cls` parameter is required.")
        url = urljoin(default_base_url().rstrip("/") + "/", url.lstrip("/"))
        headers = get_auth_header(cls.hidden.unique_id if hasattr(cls, "hidden") else "")
            ...
            async with resp:
                if resp.status >= 400:
                    ...
                    if resp.status in _RETRY_STATUS and attempt <= max_retries:
                        await sleep_with_interrupt(delay, cls)
                        delay *= retry_backoff
                        continue
