...
from .common_exceptions import ApiServerError, LocalNetworkError, ProcessingInterrupted, UnlimitAIError
...
QUEUED_STATUSES = ["created", "queued", "queueing", "submitted", "initializing", "wait", "processing"]
...
async def sync_op(
    ...
    *,
    ...
    raw = await sync_op_raw(
        cls, endpoint,
        api_key=api_key,
        price_extractor=_wrap_model_extractor(response_model, None),
        data=data, files=files, content_type=content_type,
        timeout=timeout, multipart_parser=multipart_parser,
        max_retries=max_retries, retry_delay=retry_delay,
        retry_backoff=retry_backoff, wait_label=wait_label,
        estimated_duration=estimated_duration, as_binary=False,
    ...
    if not isinstance(raw, dict):
        raise Exception("Expected JSON response to validate into a Pydantic model, got non-JSON.")
    ...
    if isinstance(data, BaseModel):
        ...
        for k, v in list(data.items()):
            if isinstance(v, Enum):
                data[k] = v.value
    cfg = _RequestConfig(
        node_cls=cls, endpoint=endpoint, api_key=api_key,
        timeout=timeout, content_type=content_type,
        data=data, files=files, multipart_parser=multipart_parser,
        max_retries=max_retries, retry_delay=retry_delay,
        retry_backoff=retry_backoff, wait_label=wait_label,
        monitor_progress=monitor_progress, estimated_total=estimated_duration,
        final_label_on_success=final_label_on_success,
        progress_origin_ts=progress_origin_ts,
...
async def poll_op_raw(
    ...
    *,
    ...
    progress_bar = utils.ProgressBar(100) if status_extractor else None
    ...
    try:
        while consumed_attempts < max_poll_attempts:
            try:
                resp_json = await sync_op_raw(
                    cls, poll_endpoint,
                    api_key=api_key,
                    data=data, timeout=timeout_per_poll,
                    max_retries=max_retries_per_poll,
                    retry_delay=retry_delay_per_poll,
                    retry_backoff=retry_backoff_per_poll,
                    wait_label="Checking", estimated_duration=None,
                    as_binary=False, final_label_on_success=None,
                ...
                if cancel_endpoint:
                    with contextlib.suppress(Exception):
                        await sync_op_raw(
                            cls, cancel_endpoint,
                            api_key=api_key,
                            timeout=cancel_timeout, max_retries=0,
                            wait_label="Cancelling task",
                            estimated_duration=None, as_binary=False,
                            final_label_on_success=None, monitor_progress=False,
                        )
                raise
            ...
            is_queued = status in queued_states if status is not None else True
            ...
            if status in completed_states:
                ...
                _display_time_progress(
                    cls, status=status or "Completed",
                    elapsed_seconds=int(now_ts - started),
                    estimated_total=estimated_duration, price=state.price,
                    is_queued=False,
            ...
            if status in failed_states:
                ...
                logging.error(msg)
                raise UnlimitAIError(msg)

            try:
                await sleep_with_interrupt(poll_interval, cls)
            except ProcessingInterrupted:
                if cancel_endpoint:
                    with contextlib.suppress(Exception):
                        await sync_op_raw(
        ...
        raise UnlimitAIError(f"Polling aborted due to error: {e}") from e
    finally:
        stop_ticker.set()
        with contextlib.suppress(Exception):
            await ticker_task
...
async def _diagnose_connectivity() -> dict[str, bool]:
    results = {"internet_accessible": False, "api_accessible": False}
...
def _friendly_http_message(status: int, body: Any) -> str:
    if status == 401:
        return "Unauthorized: Please check your UnlimitAI API Key."
    if status == 402:
        return "Payment Required: Please add credits to your UnlimitAI account."
    if status == 409:
        return "There is a problem with your account. Please contact support."
    if status == 429:
...
async def _request_base(cfg: _RequestConfig, expect_binary: bool):
    ...
    while True:
        ...
        payload_headers.update(get_auth_header(cfg.api_key))
        ...
        try:
                ...
                if cfg.files:
                    ...
                    for field_name, file_obj in file_iter:
                        if file_obj is None:
                            continue
                        if isinstance(file_obj, tuple):
                            filename, file_value, ct = _unpack_tuple(file_obj)
                        else:
                            filename = getattr(file_obj, "name", field_name)
                            file_value = file_obj
                        ...
                        if isinstance(file_value, BytesIO):
                            with contextlib.suppress(Exception):
                                file_value.seek(0)
                        form.add_field(field_name, file_value, filename=filename, content_type=ct)
            ...
            request_logger.log_request_response(
                operation_id=operation_id, request_method=method,
                request_url=url, request_headers=dict(payload_headers) if payload_headers else None,
            ...
            async with resp:
                if resp.status >= 400:
                    ...
                    if should_retry:
                        logging.warning("HTTP %s %s -> %s. Waiting %.2fs.", method, url, resp.status, wait_time)
                        request_logger.log_request_response(
                            operation_id=operation_id, request_method=method,
                            request_url=url, response_status_code=resp.status,
                            response_headers=dict(resp.headers), response_content=body,
                            error_message=f"HTTP {resp.status} (will retry in {wait_time:.1f}s)",
                        )
                        await sleep_with_interrupt(wait_time, cfg.node_cls)
                        continue
                    ...
                    raise UnlimitAIError(msg, status_code=resp.status, response_body=body)
                ...
                if expect_binary:
                    ...
                    request_logger.log_request_response(
                        operation_id=operation_id, request_method=method,
                        request_url=url, response_status_code=resp.status,
                        response_headers=dict(resp.headers), response_content=bytes_payload,
                    )
                    ...
                    request_logger.log_request_response(
                        operation_id=operation_id, request_method=method,
                        request_url=url, response_status_code=resp.status,
                        response_headers=dict(resp.headers), response_content=response_content_to_log,
                    )
                    return payload

        except ProcessingInterrupted:
            logging.debug("Request was interrupted by user")
            raise
        except (ClientError, OSError) as e:
            if (attempt - rate_limit_attempts) <= cfg.max_retries:
                logging.warning("Connection error %s %s. Retrying in %.2fs: %s", method, url, delay, str(e))
                request_logger.log_request_response(
                    operation_id=operation_id, request_method=method,
                    request_url=url, error_message=f"{type(e).__name__}: {str(e)} (will retry)",
                )
                await sleep_with_interrupt(delay, cfg.node_cls)
                delay *= cfg.retry_backoff
                continue
            ...
            if operation_succeeded and cfg.monitor_progress and cfg.final_label_on_success:
                _display_time_progress(
                    ...
                    elapsed_seconds=final_elapsed_seconds if final_elapsed_seconds is not None else int(time.monotonic() - start_time),
                    estimated_total=cfg.estimated_total, price=extracted_price, is_queued=False,
...
def _validate_or_raise(response_model: type[M], payload: Any) -> M:
    try:
        ...
        logging.error("Response validation failed for %s: %s", getattr(response_model, "__name__", response_model), e)
        raise UnlimitAIError(
